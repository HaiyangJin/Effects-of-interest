...
- (correct) Response times (RT, analyzed with log-normal transformation; `logrt`)

## Data simulation

Two data sets were simulated for behavioral choices and response times separately. There were 30 participants in each group. For simplicity, the average performance for each participant was simulated directly. Please refer to the codes in the open materials for detailed simulation procedures. Table \@ref(tab:simu-true) displays the ground truth for each condition, and the structure of the simulated data is as follows:
...
The structure of the simulated data _d_' is:
```{r}
str(df_d)
```
...
tibble(Test = c("pre-test", "post-test"),
       ...
       rt_control = c(round(exp(logrt_pre)), round(exp(logrt_delta[1]))),
       rt_train = c(round(exp(logrt_pre)), round(exp(logrt_delta[2])))) %>% 
...
The alpha level of 0.05 would be used at the single test level for all the single tests, and, therefore, the Conclusion-based Type I Error Rate (CBER) is not higher than 0.05 (see Appendix C).  

### Sensitivity d'

A 2 $\times$ 2 mixed-ANOVA was first performed on sensitivity _d_' with `library(afex)`:

```{r echo=TRUE}
d_aov <- aov_4(d ~ Group * Test + (Test | Subj), df_d)
```

The results are not displayed as none of these effects could test the directional effects  specified in EOI. Thus, we use `library(emmeans)` to, first, test one of the simple effects (i.e., post-test vs. pre-test in the training group):
...
The above result shows that the _d_' of the post-test was significantly better than the pre-test in the training group. Next, we tested the two-way interaction (i.e., the increase of _d_' in the training group relative to the control group):

```{r echo=TRUE}
summary(contrast(d_emm, interaction="revpairwise"), side = ">", infer=T)
```
This result shows that the increase of _d_' from pre- to post-test was significantly greater in the training group relative to the control group. 

### Correct response times

A similar 2 $\times$ 2 mixed-ANOVA was performed on correct response times (RT) with `library(afex)`:

```{r echo=TRUE}
logrt_aov <- aov_4(logrt ~ Group * Test + (Test | Subj), df_rt)
```

Next, we test one of the simple effects (i.e., post-test vs. pre-test in the training group):
...
This result shows that RT in post-test is shorter than the pre-test in the training group. Next, we examined the two-way interaction (i.e., the decrease of RT in the training group relative to the control group):

```{r echo=TRUE}
summary(contrast(logrt_emm, interaction="revpairwise"), side = "<", infer=T)
```
The magnitude of the RT decrease was not significantly greater in the training relative to the control group. Therefore, the RT results did not provide the evidence that the training is effective. Notably, this result does not suggest that the training protocol was not effective, either.

In summary, with both the results of _d_' and RT as specified in EOI, we may claim that the training protocol is effective.


# (Re-)analyze the simulated data with multilevel models

Specifying EOI is not specific to any statistical method. Appendix A demonstrated how to employ ANOVA to inspect the EOI. In this appendix, multilevel models (also known as linear-mixed effects models or hierarchical models) are used to inspect the same EOI with the same simulated data set. Following the analysis in Appendix A, behavioral choices were also analyzed with signal detection theory (sensitivity _d_') and correct responses times were analyzed with log-normal transformation. Of note, it is typically more beneficial to perform multilevel models on the trial-level data and incorporate the full random-effect structure. For simplicity, multilevel models employed in this appendix will only incorporate the by-subject random intercept and be performed on the condition means for each participant. Anyhow, this simplification would not invalidate the following demonstration. Please refer to the open materials of @jinTwoFacesHolistic for more appropriate applications of multilevel models with signal detection theory and log-transformation.

## Sensitivity d'

The same EOI would be inspected here: (1) _d_' in the post- relative to the pre-test in the training group, and (2) the increase of _d_' in the training group relative to the control group. It happened that these two effects could be tested with the default dummy contrast coding in R when the default level in each factor was set appropriately.
...
These results suggest that in the training group, the _d_' in the pre-test was worse than that in the post-test (results of `Testpre`) and the increase of _d_' in the training group was greater than the control group (results of `Groupcontrol:Testpre`). Noteworthy that these results (mainly the p-values) were from two-sided tests. Since the effects in EOI were directional, we might use one-sided instead of two-sided tests. For example, we might use a t-value of 1.65 instead of 1.96 as the threshold to claim statistical significance. Alternatively, `library(emmeans)` could be used to perform the one-side tests, similar to Appendix A. 
...
The above result shows that the _d_' in the post-test was significantly better than the pre-test in the training group. Next, we tested the two-way interaction (i.e., the increase of _d_' in the training group relative to the control group):
...
The similar multilevel model for RT is as followings:
...
Note that the above results were from two-sided tests. Therefore, we used `library(emmeans)` to perform the one-side tests. 
...
This result shows that the _d_' of the post-test was significantly better than the pre-test in the training group. Next, we tested the two-way interaction:
